...
{:start="6"}
1. [[Class 6] Pythonプログラムを用いた移動指示とマニピュレータ操作](#class-6-pythonプログラムを用いた移動指示とマニピュレータ操作)
   1. [Pythonプログラムでの地図上座標の取得](#pythonプログラムでの地図上座標の取得)
      1. [`rostopic echo` を用いる場合](#check-clicked-point-by-rostopic)
      1. [Pythonプログラムを用いた座標の取得](#pythonプログラムを用いた座標の取得)
      1. [ファイルの内容確認](#ファイルの内容確認)
      1. [Pythonプログラムのインタラクティブな実行](#pythonプログラムのインタラクティブな実行)

   1. [プログラム内に記述された座標への移動](#プログラム内に記述された座標への移動)
      1. [目標位置付近で停止せず回転し続ける問題](#目標位置付近で停止せず回転し続ける問題)
      1. [経路計画のパラメータ変更](#経路計画のパラメータ変更)
         1. [`rqt_reconfigure` でのパラメータ変更](#rqt_reconfigure-でのパラメータ変更)
         1. [`rosparam` コマンドでのパラメータ変更](#rosparam-コマンドでのパラメータ変更)
      1. [課題](#課題)

   1. [RViz上で指定した位置への移動](#rviz上で指定した位置への移動)

   1. [アームの操作](#アームの操作)
      1. [アーム動作の前準備](#アーム動作の前準備)
         1. [Gazeboの場合](#gazeboの場合)
         1. [実機の場合](#実機の場合)
      1. [`move_arm.py`](#move_armpy)

   1. [RVizで指定された点にある棒を倒すプログラム](#rvizで指定された点にある棒を倒すプログラム)
      1. [Gazebo環境の物体配置初期化](#gazebo環境の物体配置初期化)
      1. [課題](#課題-1)

   1. [RVizで指定された点にある棒を倒すプログラム(連続動作版)](#exercise6-6)

   1. [発展課題 円形障害物を検知しその近くに移動するプログラム](#発展課題-円形障害物を検知しその近くに移動するプログラム)

   1. [発展課題 円形障害物を検知しそれを倒すプログラム](#exercise6-ex2)
