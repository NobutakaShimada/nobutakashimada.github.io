...
{:id="exercise1-1"}
{% capture exercise1-1 %}
リモコン操作でロボットの動きをよく観察しなさい。リモコンからの入力はどういう信号に変換されて送られていると思うか（位置、速度、加速度など）。各キーにバインドされた制御信号を予想してレポート（latex）に記載しなさい。移動のための車輪の操作とアームの操作で、指令入力の性質が違うかもしれない。どのような動きからそう予想するのか、考察も含めて解答せよ。
{% endcapture %}
...
{:id="exercise1-4"}
{% capture exercise1-4 %}
1. 授業中の解説を参考に、キーボードで入力したテキスト文字を送信するROSノードtalker2.pyをpython(rospy)を使って実装しなさい。python2ではキーボード入力の取得にraw_input()が使える。
2. talker2.pyのコードをレポートファイルに添付しなさい。
3. テキストの送受信に使用されるトピック名と、送受信メッセージの定義（rosmsgコマンドを使用せよ、使い方はチュートリアルにある）を調べて、コマンドレポートファイルに記載せよ。
...
{:id="exercise1-5"}
{% capture exercise1-5 %}
1. talker2.pyとlistener.pyのpythonコードを参考に、一つのコードで双方向にテキストを「送受信」できるノードに改造した.pyを実装せよ。
...
- うまくいった場合，起動したノードを`Ctrl-C`で停止したのち，rqt_graphコマンドを実行すると他に繋いでいる人のノードが見えるはずである．確認せよ．
